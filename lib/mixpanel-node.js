...
var create_client = function(token, config) {   
    // TODO: See where this file ends up.    
    var db = low('./request_queue.db', {async: true});
    ...
    */
    metrics.send_request = function(endpoint, data, callback, retry) {
        callback = callback || function() {};
        retry = retry || false;
        var event_data = new Buffer(JSON.stringify(data));
        var request_data = {
            'data': event_data.toString('base64'),
            // We want to store the IP of the request
            'ip': 1,
    ...
    */
    ...
    /**
        ...
        ---
        Attempts to resend events that were triggered offline.
    */
    metrics.sync_offline_requests = function() {
        db('request_queue').forEach(function(request) {
            metrics.send_request(request.endpoint, request.data, function(e) {
                if (!e) { // if success
                    db('request_queue').remove(request);
                }
            }, true); // retry param prevents request reinsertion
        });
    };
